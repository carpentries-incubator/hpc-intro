...
```bash
`r config$remote$prompt` set
```

```output
HOME=`r config$remote$home`/`r config$remote$user`
HOSTNAME=`r config$remote$host`
HOSTTYPE=`r config$remote$hosttype`
PATH=`r config$remote$home`/`r config$remote$user`/bin:/usr/local/git/bin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin
PWD=`r config$remote$home`/`r config$remote$user`
UID=1000
USER=`r config$remote$user`
...
```
...
```bash
`r config$remote$prompt` echo HOME
```
...
Thanks to this expansion, what we actually run is ``echo `r config$remote$homedir`/`r config$remote$user` ``,
...
and `` `r config$remote$home`/`r config$remote$user`/analyze ``.
Since the shell searches the directories in the order they're listed in `PATH`,
it finds `/bin/analyze` first and runs that.
Notice that it will *never* find the program `` `r config$remote$home`/`r config$remote$user`/analyze ``
unless we type in the full path to the program,
since the directory `` `r config$remote$home`/`r config$remote$user` `` isn't in `PATH`.

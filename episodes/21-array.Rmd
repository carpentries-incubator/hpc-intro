...
the work may be divided into cooperating subtasks that communicate with
...
multiple processes, and having these processes cooperate by exchanging
information during execution using the Message Passing Inteface. This
is the "MPI" that we saw as a dependency of the `am

Not all workloads require processes to cooperate. Many scientific workflows
are made up of independent serial jobs. For these cases, `r config$sched$name`
...
We have aleady seen how to submit a single seriial job to the
Slurm scheduler.

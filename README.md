...
2. For easier portability, we use snippets of text and code to capture inputs
   ...
   - Code snippets are placed in subdirectories that are named according to the
     episode they appear in. For example, if the snippet is for episode 12,
     then it will be in a subdirectory called `12`.
   - In the episodes source, snippets are included using [Liquid](
     https://shopify.github.io/liquid/) scripting `include` statements. For
     example, the first snippet in episode 12 is included using `{% include
     /snippets/12/info.snip %}`.
...
1. [Why use a cluster?](_episodes/11-hpc-intro.md) (20 minutes)
    * Brief, concentrate on the concepts not details like interconnect type,
      etc.
...
2. [Working on a remote HPC system](_episodes/12-cluster.md) (35 minutes)
    ...
    * Objectives: Connect to a cluster using ssh; Transfer files to and from
      the cluster; Run the hostname command on a compute node of the cluster.
...
3. [Working with the scheduler](_episodes/13-scheduler.md) (1 hour 15 minutes)
    ...
    * Potential tools: shell script, `sbatch`, `squeue -u`, `watch`, `-N`,
      `-n`, `-c`, `--mem`, `--time`, `scancel`, `srun`, `--x11 --pty`,
...
5. [Transferring files with remote
   computers](_episodes/15-transferring-files.md) (30 minutes)
    ...
    * Know what tools can be used for file transfers, and transfer modes
      (binary vs text)
...
8. [Using shared resources responsibly](_episodes/18-responsibility.md) (20
   minutes)
...
* Playing friendly in the cluster (psteinb: the following is very tricky as it
...
* Filesystems and Storage: objectives likely include items from @psteinb's

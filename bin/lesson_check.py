...
BREAK_METADATA_FIELDS = {("layout", str), ("title", str), ("break", int)}
...
def parse_args():
    ...
    parser.add_argument(
        "-p", "--parser", default=None, dest="parser", help="path to Markdown parser"
    )
    parser.add_argument(
        "-r",
        "--references",
    ...
    parser.add_argument(
        "-s", "--source", default=os.curdir, dest="source_dir", help="source directory"
    )
    parser.add_argument(
        "-w",
        "--whitespace",
    ...
    require(
        not extras, 'Unexpected trailing command-line arguments "{0}"'.format(extras)
    )
...
def check_config(reporter, source_dir):
    ...
    reporter.check_field(
        config_file, "configuration", config, "carpentry", ("swc", "dc", "lc", "cp")
    )
...
class CheckBase:
    ...
    def check_metadata(self):
        ...
        self.reporter.check(
            self.metadata is not None, self.filename, "Missing metadata entirely"
        )
    ...
    def check_line_lengths(self):
        ...
        if self.args.line_lengths:
            over = [
                ...
                if (n > MAX_LINE_LEN) and (not l.startswith("!"))
            ]
            self.reporter.check(
                not over,
                self.filename,
...
class CheckNonJekyll(CheckBase):
    ...
    def check_metadata(self):
        self.reporter.check(self.metadata is None, self.filename, "Unexpected metadata")
...
class CheckIndex(CheckBase):
    ...
    def check_metadata(self):
        ...
        self.reporter.check(
            self.metadata.get("root", "") == ".", self.filename, 'Root not set to "."'
        )
...
class CheckEpisode(CheckBase):
    ...
    def check_metadata_fields(self, expected):
        ...
        for (name, type_) in expected:
            if name not in self.metadata:
                self.reporter.add(self.filename, "Missing metadata field {0}", name)
    ...
    def check_reference_inclusion(self):
        ...
        require(last_line, "No non-empty lines in {0}".format(self.filename))

        include_filename = os.path.split(self.args.reference_path)[-1]
        if include_filename not in last_line:
            self.reporter.add(
                self.filename, 'episode does not include "{0}"', include_filename
            )
